...
class MainWidget(RelativeLayout):
    ...
    v_no_lines = 8
    v_lines_spacing = .4 #percentage in screen in width
    vertical_lines = []
    ...
    score = StringProperty("S C O R E : 0")
    ...
    def update_ship(self):
        ...
        ship_half_wid = self.ship_width * self.width /2
        ship_hei = self.ship_height * self.height

        self.ship_coordinates[0]= (center_x -ship_half_wid, base_y)
        self.ship_coordinates[1]= (center_x , base_y+ ship_hei)
        self.ship_coordinates[2]= (center_x + ship_half_wid, base_y)
    ...
    def gererate_titles_coordinates(self):
        last_y = 0
        last_x = 0
    ...
    def init_vertical_lines(self):
        with self.canvas:
            ...
            for i in range(self.v_no_lines):
    ...
    def update_tiles(self):
        for i in range(0, self.no_tiles):
            ...
            xmin, ymin = self.get_tile_coordinate(tile_coordinate[0],tile_coordinate[1])
            xmax, ymax = self.get_tile_coordinate(tile_coordinate[0]+1,tile_coordinate[1]+1)
    ...
    def init_horizontal_lines(self):
        with self.canvas:
            Color(1,1,1)
            #self.l ine = Line(points=[self.width/2,0,self.width/2,self.height])
            for i in range(self.h_no_lines):
    ...
    def update(self, dt):
        ...
        if not self.check_ship_collision()  and not self.state_game_over:
            ...
            Clock.schedule_once(self.play_game_over_voice_sound, 0.2)
